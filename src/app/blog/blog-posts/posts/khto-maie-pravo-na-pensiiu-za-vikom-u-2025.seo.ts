export const khto_maie_pravo_na_pensiiu_za_vikom_u_2025 = {
  'khto-maie-pravo-na-pensiiu-za-vikom-u-2025': {
    title:
      'Хто має право на пенсію за віком у 2025 році? | Адвокат Поддяча Юлія Юріївна',
    description:
      'Хто має право на пенсію за віком у 2025 році: який страховий стаж потрібен для виходу у 60, 63 або 65 років, посилання на ст. 26 Закону №1058-IV та що робити у разі відмови Пенсійного фонду.',
    keywords:
      'право на пенсію 2025, пенсія за віком, страховий стаж 2025, пенсія в 60 років, пенсія в 63 роки, пенсія в 65 років, юридична допомога з пенсійних питань, адвокат по пенсіях',
    type: 'article',
    publishedTime: '2025-09-28T19:00:00+03:00',
    modifiedTime: '2025-09-28T19:00:00+03:00',
    canonical: '/blog/khto-maie-pravo-na-pensiiu-za-vikom-u-2025',
    schema: {
      '@context': 'https://schema.org',
      '@graph': [
        {
          '@type': 'BlogPosting',
          headline: 'Хто має право на пенсію за віком у 2025 році?',
          description:
            'Право на пенсію за віком у 2025 році: який страховий стаж потрібен для виходу у 60, 63 чи 65 років, посилання на ст. 26 Закону №1058-IV і що робити при відмові ПФУ.',
          datePublished: '2025-09-28T19:00:00+03:00',
          dateModified: '2025-09-28T19:00:00+03:00',
          author: {
            '@type': 'Person',
            name: 'Поддяча Юлія Юріївна',
            url: 'https://www.advocate-pensia.com.ua/about-me',
          },
          publisher: {
            '@type': 'Organization',
            name: 'Поддяча Юлія Юріївна',
            logo: {
              '@type': 'ImageObject',
              url: 'https://www.advocate-pensia.com.ua/assets/logo/logo-black-cut.png',
              width: 512,
              height: 512,
            },
          },
          image: [
            'https://www.advocate-pensia.com.ua/assets/images/og/default-og-image.webp',
          ],
          inLanguage: 'uk-UA',
          keywords: [
            'право на пенсію 2025',
            'пенсія за віком',
            'страховий стаж',
            'пенсія в 60 років',
            'пенсія в 63 роки',
            'пенсія в 65 років',
            'юридична допомога з пенсійних питань',
          ],
          articleSection: 'Пенсійне право',
          mainEntityOfPage: {
            '@type': 'WebPage',
            '@id':
              'https://www.advocate-pensia.com.ua/blog/khto-maie-pravo-na-pensiiu-za-vikom-u-2025',
          },
          url: 'https://www.advocate-pensia.com.ua/blog/khto-maie-pravo-na-pensiiu-za-vikom-u-2025',
        },
        {
          '@type': 'BreadcrumbList',
          itemListElement: [
            {
              '@type': 'ListItem',
              position: 1,
              name: 'Блог',
              item: 'https://www.advocate-pensia.com.ua/blog',
            },
            {
              '@type': 'ListItem',
              position: 2,
              name: 'Хто має право на пенсію за віком у 2025 році?',
              item: 'https://www.advocate-pensia.com.ua/blog/khto-maie-pravo-na-pensiiu-za-vikom-u-2025',
            },
          ],
        },
      ],
    },
  },
};
