export const pilhova_pensiia_vodiiam_miskoho_pasazhyrskoho_transportu_avtobusiv_trolejbusiv_tramvaiv_i_velykovahovykh_avtomobiliv =
  {
    'pilhova-pensiia-vodiiam-miskoho-pasazhyrskoho-transportu-avtobusiv-trolejbusiv-tramvaiv-i-velykovahovykh-avtomobiliv':
      {
        title: 'Пільгова пенсія водіям міського транспорту | Advocate Pensia',
        description:
          'Умови призначення пільгової пенсії водіям автобусів, тролейбусів і трамваїв. Порівняння законодавства, вимоги до стажу та судова практика.',
        keywords:
          'пільгова пенсія водіям, пенсія водіям міського транспорту, пенсія водіям автобусів, пенсія водіям тролейбусів, пенсія водіям трамваїв, стаття 13 закону 1788-XII, стаття 114 закону 1058-IV, пенсія за шкідливі умови праці',
        type: 'article',
        publishedTime: '2025-11-26T09:00:00+02:00',
        modifiedTime: '2025-11-26T09:00:00+02:00',
        canonical:
          'https://www.advocate-pensia.com.ua/blog/pilhova-pensiia-vodiiam-miskoho-pasazhyrskoho-transportu-avtobusiv-trolejbusiv-tramvaiv-i-velykovahovykh-avtomobiliv',
        schema: {
          '@context': 'https://schema.org',
          '@graph': [
            {
              '@type': 'BreadcrumbList',
              itemListElement: [
                {
                  '@type': 'ListItem',
                  position: 1,
                  name: 'Головна',
                  item: 'https://www.advocate-pensia.com.ua',
                },
                {
                  '@type': 'ListItem',
                  position: 2,
                  name: 'Блог',
                  item: 'https://www.advocate-pensia.com.ua/blog',
                },
                {
                  '@type': 'ListItem',
                  position: 3,
                  name: 'Пільгова пенсія водіям міського транспорту',
                },
              ],
            },
            {
              '@type': 'WebPage',
              '@id':
                'https://www.advocate-pensia.com.ua/blog/pilhova-pensiia-vodiiam-miskoho-pasazhyrskoho-transportu-avtobusiv-trolejbusiv-tramvaiv-i-velykovahovykh-avtomobiliv#webpage',
              url: 'https://www.advocate-pensia.com.ua/blog/pilhova-pensiia-vodiiam-miskoho-pasazhyrskoho-transportu-avtobusiv-trolejbusiv-tramvaiv-i-velykovahovykh-avtomobiliv',
              name: 'Пільгова пенсія водіям міського транспорту | Advocate Pensia',
              description:
                'Умови призначення пільгової пенсії водіям автобусів, тролейбусів і трамваїв. Порівняння законодавства, вимоги до стажу та судова практика.',
              inLanguage: 'uk-UA',
              isPartOf: {
                '@id': 'https://www.advocate-pensia.com.ua#website',
              },
              breadcrumb: {
                '@id':
                  'https://www.advocate-pensia.com.ua/blog/pilhova-pensiia-vodiiam-miskoho-pasazhyrskoho-transportu-avtobusiv-trolejbusiv-tramvaiv-i-velykovahovykh-avtomobiliv#breadcrumb',
              },
              datePublished: '2025-11-26T09:00:00+02:00',
              dateModified: '2025-11-26T09:00:00+02:00',
            },
            {
              '@type': 'BlogPosting',
              '@id':
                'https://www.advocate-pensia.com.ua/blog/pilhova-pensiia-vodiiam-miskoho-pasazhyrskoho-transportu-avtobusiv-trolejbusiv-tramvaiv-i-velykovahovykh-avtomobiliv#blogposting',
              headline: 'Пільгова пенсія водіям міського транспорту',
              description:
                'Умови призначення пільгової пенсії водіям автобусів, тролейбусів і трамваїв. Порівняння законодавства, вимоги до стажу та судова практика.',
              datePublished: '2025-11-26T09:00:00+02:00',
              dateModified: '2025-11-26T09:00:00+02:00',
              dateCreated: '2025-05-12T09:00:00+03:00',
              author: {
                '@type': 'Person',
                name: 'Ігор Шевчук',
              },
              publisher: {
                '@type': 'Organization',
                name: 'Advocate Pensia',
                url: 'https://www.advocate-pensia.com.ua',
              },
              inLanguage: 'uk-UA',
              mainEntityOfPage: {
                '@type': 'WebPage',
                '@id':
                  'https://www.advocate-pensia.com.ua/blog/pilhova-pensiia-vodiiam-miskoho-pasazhyrskoho-transportu-avtobusiv-trolejbusiv-tramvaiv-i-velykovahovykh-avtomobiliv',
              },
              keywords: [
                'пільгова пенсія водіям',
                'пенсія водіям міського транспорту',
                'пенсія водіям автобусів',
                'пенсія водіям тролейбусів',
                'пенсія водіям трамваїв',
                'стаття 13 закону 1788-XII',
                'стаття 114 закону 1058-IV',
                'пенсія за шкідливі умови праці',
              ],
            },
          ],
        },
      },
  };
